...
- **Этап A (дома, где есть интернет)** — подготовка офлайн‑пакетов.
...
## Отдельный сценарий: Astra Linux 1.8 без GUI и без Docker

Если целевая машина:

- без графического интерфейса,
- без Docker,
- “голая” (нет Python/PostgreSQL/Redis/Nginx/Node),

то используйте документ `DEPLOY_ASTRA18_OFFLINE_NOGUI.md` — там подробно описано промышленное развёртывание через:

- systemd (Gunicorn + Celery),
- Nginx (раздача frontend + reverse proxy `/api`),
- PostgreSQL и Redis как системные сервисы.

---

## Офлайн развёртывание через Docker (сборка дома → перенос на предприятие)

Если Docker на предприятии поставить можно, используйте документ `DEPLOY_DOCKER_OFFLINE.md`.
Он описывает, как:

- дома собрать образы,
- экспортировать их в `tar`,
- на предприятии импортировать и поднять `docker compose` без интернета.
